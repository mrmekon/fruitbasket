...
impl FruitApp {
    /// Docs in OS X build.
    pub fn new() -> FruitApp { FruitApp{} }
    /// Docs in OS X build.
    pub fn set_activation_policy(&self, _policy: ActivationPolicy) {}
    /// Docs in OS X build.
    pub fn terminate(_exit_code: i32) {}
    /// Docs in OS X build.
    pub fn stop(_stopper: &FruitStopper) {}
    /// Docs in OS X build.
    pub fn run(&mut self, _period: RunPeriod) {}
    /// Docs in OS X build.
...
impl Trampoline {
    ...
    pub fn build(&mut self, _dir: InstallDir) -> Result<FruitApp, FruitError> {
        Err(FruitError::new("fruitbasket disabled or not supported on this platform."))
    }
    /// Docs in OS X build.
